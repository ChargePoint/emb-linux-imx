...
static const struct ampire_panel_cmd init_code[] = {
	...
	{ 0x10, 0x95 }, /* FRC=1, BIT_SWAP=001, RGB_SWAP=101 */
	{ 0x11, 0x88 }, /* SYS_CTRL_1 */
	{ 0xB6, 0x20 }, /* ?? */
...
static int ampire_panel_prepare(struct drm_panel *panel)
{
	...
	printk("ampire_panel_prepare: regulator_enable = %d\n", err);
	if (err < 0)
		return err;
	...
	if (ampire->i2c) {
		...
		for (i = 1; i < ARRAY_SIZE(init_code); i++) {
			printk("ampire->i2c: ICN6211{%02X <= %02X}\n",
							init_code[i].cmd, init_code[i].data);
...
static const struct drm_display_mode default_mode = {
	...
	.hdisplay = 640,
	.hsync_start = 640 + 15,
	.hsync_end = 640 + 15 + 215,
	.htotal = 640 + 15 + 215 + 15,
	.vdisplay = 480,
	.vsync_start = 480 + 3,
	.vsync_end = 480 + 3 + 34,
	.vtotal = 480 + 3 + 34 + 10,
...
static int ampire_panel_get_modes(struct drm_panel *panel)
{
	...
	panel->connector->display_info.width_mm = 117;
	panel->connector->display_info.height_mm = 92;
...
static int ampire_panel_add(struct ampire_panel *ampire)
{
	...
	drm_panel_init(&ampire->base);
	ampire->base.funcs = &ampire_panel_funcs;
	ampire->base.dev = &ampire->link->dev;

	return drm_panel_add(&ampire->base);
}
