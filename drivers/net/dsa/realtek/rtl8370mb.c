...
static_assert(ARRAY_SIZE(rtl8370mb_mib_counters) == RTL8365MB_MIB_END);
...
struct rtl8370mb {
	...
	struct rtl8370mb_port ports[RTL8365MB_MAX_NUM_PORTS];
};
...
static int initial_LED(void)
{
    ...
    dev_info(g_priv->dev, "rtk_led_outputEnable_set\n");
...
static irqreturn_t rtl8370mb_irq(int irq, void *data)
{
	...
	ret = rtl8370mb_get_and_clear_status_reg(priv, RTL8365MB_INTR_STATUS_REG,
						 &stat);
	if (ret)
		goto out_error;
	...
	if (stat & RTL8365MB_INTR_LINK_CHANGE_MASK) {
		...
		ret = rtl8370mb_get_and_clear_status_reg(
...
MODULE_DESCRIPTION("Driver for RTL8370MB-VC ethernet switch");
MODULE_LICENSE("GPL");
